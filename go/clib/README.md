# libtabnasc — the c parser as a C ABI

<!-- tabnas-clib-template: v1 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The c format parser as a C shared library, so languages with no
tabnas port can parse and validate c input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasc","format":"c","template":"v1"}` |
...
// link against libtabnasc; every call returns a JSON []u8 to free with
// c.tabnas_free.
```
