...
| [`go/`](go/) | **Go port — IN PROGRESS.** Done & tested: the lexer (`tokens.go`, `symbols.go`, `matchers.go`), CST helpers (`cst.go`), `@tabnas/expr` wiring + C-atom expressions (`expr_grammar.go`), grammar parse/install + ref-stub framework (`grammar_install.go`, `refs.go`), the `#if`-folding post-pass (`conditional_groups.go`), and the top-level chomp path (`refs.go` + a minimal `structure.go`) — the full pipeline parses C to a **token-fidelity** `translation_unit` CST with cross-declaration typedef tracking. Still to come: the structured dispatch refs (`simple_declaration`, statements, attributes, preprocessor, asm) and the full legacy structurer (`structure.go` + the hand-rolled `expr.go`), then the ported test suite + CSmith fixture parity. Refs not yet ported get typed no-op stubs so the grammar always installs. The upstream `@jsonic/c` is TypeScript-only, so the port is a from-scratch hand-translation. |
...
## Go port: remaining work for CSmith parity

The Go port's architecture is complete and it parses C into structured CSTs,
but `go test`'s `TestCsmithCorpus` parity tracker sits at ~8/100. Root cause
(diagnosed, not a mystery): the golden fixtures are generated by the TS parser
in **extended mode, which uses the grammar "new path"** (the `@looks-simple-decl`
cascade → `simple_declaration` and the struct/enum/initializer/statement
grammar rules). The Go port currently routes everything through the **legacy
chomp + `structure.go`** (the new-path dispatch refs are no-op stubs), and the
two paths are NOT byte-identical for some constructs — e.g. a bitfield width is
`literal_expression` on the new path but opaque `tok`s in the legacy
structurer; initializer items differ by a stray token. So the remaining work to
reach parity is to **port the new-path structured-dispatch ref groups** in
`makeGrammarRefs` (`c.ts` ~2630–5440): `@looks-simple-decl` + `@sd*`/`@idecl*`/
`@dr*`/`@spec*`/`@ptr*`/`@arr*` (declarations & declarators), `@gs*`/`@es*`/
`@mdl*`/`@enr*` (struct/union/enum), `@il*`/`@ii*` (initializers), the statement
groups (`@if*`/`@do*`/`@for*`/`@ss*`/`@js*`/`@lbl*`/`@cocl*`), attributes
(`@as23*`/`@ga*`/`@aal*`/`@ai*`) and asm (`@asm*`/`@asg*`/`@asec*`). Enable each
group and watch the parity rate climb; the legacy `structure.go` stays as the
long-tail fallback.
